...
public class TileEntitySteamTurbine extends TileEntityMachine {
    ...
    public TileEntitySteamTurbine() {
        super(new FluidTank[]{new FluidTank(1000)},new AlchemistryEnergyStorage(1000));
    }
    ...
    public void update() {
            ...
            for(SteamTurbineRecipe recipe : ModRecipes.steamTurbineRecipes) {
                if(recipe.match(steamTank.getFluid())) {
                    ...
                    if(recipe.canApply(steamTank.getFluid(),energyStorage)) {
                        int multiplier = Math.min(steamTank.getFluidAmount() / recipe.input.amount,(energyStorage.getMaxEnergyStored() - energyStorage.getEnergyStored()) / recipe.output);
                        steamFlow += recipe.input.amount * multiplier;
                        steamTank.drain(recipe.input.amount * multiplier,true);
                        energyStorage.receiveEnergy(recipe.output * multiplier,false);
                    }
                    break; // 只匹配第一个可用配方
                }
            }
        ...
        for(EnumFacing facing : EnumFacing.values()) {
            ...
            if(tile != null) {
                EnergyUtil.transfer(energyStorage,tile,facing.getOpposite(),false);
            }
        }
        markDirty();
    }
